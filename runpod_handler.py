"""
RunPod Serverless Handler for Parakeet Transcription Service

This handler provides a serverless interface for audio transcription using
NVIDIA's Parakeet-TDT model with optional speaker diarization.
...
def initialize():
    ...
    try:
        ...
        if hf_token:
            logger.info("HuggingFace token found - speaker diarization available")
        else:
...
def handler(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        temp_dir = tempfile.mkdtemp(prefix="runpod_audio_")
        ...
        if diarize:
            ...
            if hf_token:
                ...
                # Unload diarization pipeline to free GPU memory
                diarizer.cleanup()
